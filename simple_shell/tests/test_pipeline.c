...
main(void)
{
	struct pipeline* my_pipeline = pipeline_build("pipefrom| pipeto\n");
	...
	// Test that there is only two parsed command in the pipeline
	TEST_ASSERT(my_pipeline->commands->next != NULL);
    TEST_ASSERT(my_pipeline->commands->next->next == NULL);

	// Test the parsed args
	TEST_ASSERT(strcmp("pipeto", my_pipeline->commands->next->command_args[0]) == 0);
